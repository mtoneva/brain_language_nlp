...
if __name__ == '__main__':
    ...
    text_array = np.load('/usr0/home/mktoneva/meg_net/words_fmri.npy')
    remove_chars = [",","\"","@"]
    ...
    if args.nlp_model == 'bert':
        ...
        nlp_features = get_bert_layer_representations(args.seq_len, text_array, remove_chars, word_ind_to_extract)
    elif args.nlp_model == 'transformer_xl':
        word_ind_to_extract = -1
        nlp_features = get_xl_layer_representations(args.seq_len, text_array, remove_chars, word_ind_to_extract)
    elif args.nlp_model == 'elmo':
        nlp_features = get_elmo_layer_representations(args.seq_len, text_array, remove_chars, word_ind_to_extract)
    elif args.nlp_model == 'use':
        nlp_features = get_use_layer_representations(args.seq_len, text_array, remove_chars, word_ind_to_extract)
    else:
        print('Unrecognized model name {}'.format(args.nlp_model)
               
    save_layer_representations(nlp_featurs, args.nlp_model, args.sequence_length, args.output_dir)
